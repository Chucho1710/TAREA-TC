...
public class Controlador {
    ...
    public void agregarAFND(){
        ...
        model.clear(); // Este método elimina todos los elementos
        vista.getCanvas1().limpiar();
        modelo.limpiarTrans();
        //vista.getjListTransiciones().getModel().removeListDataListener(l);
    ...
    public void convertir(){
        if(modelo.esAFD())
            JOptionPane.showMessageDialog(null, "el automata ingresado ya es un AFD no es necesario hacer la conversion");
        else {
            modelo.convertir();
            mostrarAFD();
        }
